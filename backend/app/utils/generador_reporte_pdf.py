...
def _draw_bloque(c: canvas.Canvas, pasante: dict, filas: list,
    ...
    c.drawString(x + 0.4*cm, total_top - TOTAL_H + 0.28*cm,
                 f"Total: {len(filas)} d?as  ?  {total_horas:.1f} horas acumuladas  ?  "
...
def generar_reporte_diario(pasante: dict, filas: list, titulo_diario: str) -> io.BytesIO:
    ...
    centrando el bloque verticalmente para que no se vea 'perdido' en la p?gina.
    """
    buf = io.BytesIO()
    ...
    _draw_bloque(c, pasante, filas, titulo_diario, y_top)

    ...
    <transition name="modal">
      <div v-if="showModalReporte"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
          ...
          <!-- Cuerpo formulario -->
          <div class="px-6 py-5">
            <p class="text-gray-700 font-semibold mb-1">Reporte de actividades del día</p>
            <p class="text-gray-400 text-xs mb-4">
              Describe brevemente qué hiciste hoy. Esto quedará registrado para tu encargado.
            </p>

            <textarea
              v-model="actividades"
              rows="5"
              placeholder="Ej: Apoyé en la organización del archivo, redacté 3 oficios, asistí a reunión de coordinación..."
...
const fichar = async () => {
  ...
  try {
    ...
    if (modo.value === 'entrada') {
      // Entrada → solo mostrar mensaje en pantalla
      mensaje.value = {
      ...
      // Salida → guardar datos y abrir modal de reporte
...
const enviarReporte = async () => {
  ...
  try {
    ...
    mensaje.value = {
      ...
      detalle: `Gracias, ${datosUltimaSalida.value.nombre}. Tu encargado lo revisará pronto.`
    }
...
const omitirReporte = () => {
  ...
  esError.value = false
  mensaje.value = {
    titulo:  `👋 ¡Hasta luego, ${datosUltimaSalida.value?.nombre}!`,

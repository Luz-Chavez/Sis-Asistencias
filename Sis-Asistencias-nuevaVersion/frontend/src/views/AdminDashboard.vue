<template>
  ...
  <div class="flex justify-end gap-3 items-center">
    ...
    <button @click.stop="abrirModalEvaluar(reporte)"
      :class="(reporte.estado_encargado || '').toUpperCase() === 'VERIFICADO' 
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-blue-950/80 px-4 backdrop-blur-sm overflow-y-auto">
      <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-xl my-8 overflow-hidden transform transition-all border border-blue-100">
        ...
        <div class="p-8">
          <div v-if="(reporteAEvaluar?.estado_encargado || '') === 'PENDIENTE'" class="mb-6 bg-amber-50 border border-amber-200 p-4 rounded-xl flex gap-3 text-amber-800 text-sm shadow-inner">
            ...
            <div>
              ...
              <div class="flex gap-4">
                <label class="flex-1 cursor-pointer">
                  <input type="radio" v-model="formEvaluacion.estado" value="APROBADO" :disabled="(reporteAEvaluar?.estado_encargado || '') === 'PENDIENTE'" class="peer sr-only">
...
const abrirModalEvaluar = (reporte) => {
  ...
  formEvaluacion.value = { estado: initialState, comentarios_director: '', es_rectificacion: false }
  mensajeError.value = ''
